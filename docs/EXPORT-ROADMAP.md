...
| 4 | **CNN + transformer + CTC** | wav2vec2, data2vec, hubert, omniasr-ctc ×2, tada-aligner | 3 | ~6 | CTC head (already done for family 1) | **done** (P5, 2026-09-12): `ctc_asr_export.py`, ONE generic recognizer claiming any `*ForCTC` directory, registered `fallback=True` so it cannot race the three NeMo ones on the same task. The head half of the estimate above held exactly — `CtcGreedyBuilder`, `loom.argmax_rows` and the five-statement driver are family 1's, unchanged — and **"family-1-shaped" did not**: family 1's `build_trace` is a NeMo `(input_signal, input_signal_length)` pair around a mel front end, and this family takes a raw waveform and no length at all, so it is a sibling template and what the two share is the epilogue. Four things are its own: the feature extractor's `do_normalize` goes INSIDE the graph (family 1's front-end rule; leaving it out feeds a correctly-shaped graph audio at the wrong scale and transcribes plausible nonsense); the blank is the tokenizer's `pad_token` id, NOT `num_classes - 1`, and not always spelled `<pad>` (omniASR spells it `<s>` and has a separate unused `<pad>`); no attention mask at all, for loom.cpp ADR-019's reason one modality over; and one engine reader, `CtcVocab` / `tokenizer.ggml.model == "ctc"` (loom.cpp ADR-033 for why a tag of its own rather than a `"t5"` file with the word delimiter rewritten to U+2581). **The real cost was in none of the scoping**: `topology_ops._op_conv` had read `groups > 1` as "depthwise" since the first export, which is right at both ends of the range and wrong in the middle — a `Wav2Vec2PositionalConvEmbedding` is `groups=16` over 768 channels and aborted the ENGINE inside `ggml_im2col`, and the fix for that then read a permuted activation through a `ggml_view_3d` that assumes `nb[0] == type size` (loom.cpp Retro-046). Verified against `transformers` on the LOGITS over 549 frames of real speech, on three structurally different checkpoints: `data2vec-audio-base-960h` (max \|Δ\| 1.87e-03), `hubert-large-ls960-ft` (1.73e-03) and `omniASR-CTC-300M-v2` (5.45e-04), cosine ≥ 0.99999982, 549/549 argmax each, sabotage arm 32.7 — and the f64 arm is why those deltas are accumulation and not a defect (torch's own f32 is 7.0e-04 from f64 where loom is 1.2e-03). omniASR is verified and NOT shipped: it is scale-sensitive and its own documented processor path transcribes garbage, which loom reproduces exactly |
| 5 | **SANM / FunASR encoders** (+ CIF, + CTC) | funasr ×2, paraformer, sensevoice | 3 | ~4 | CIF predictor + NAR decoder (paraformer only) | **first leaf done** (P5, 2026-09-15): `sanm_asr_export.py`, one recognizer claiming a FunASR directory whose `config.yaml` declares `model: SenseVoiceSmall`. The family-4 correction held a second time -- the CTC epilogue is family 1's, reused unchanged, and the encoder template is shared with nothing; three siblings, one epilogue. It needed **no engine primitive and no new head**. What it needed instead was in neither estimate: FunASR's `WavFrontend` does not trace (kaldi fbank frames with `as_strided` over computed strides; `apply_lfr` pads by an amount that depends on the frame count modulo the LFR stride), so the front end is rebuilt -- kaldi's per-frame DC removal, pre-emphasis, windowing and zero-padding are all LINEAR and fold into one strided convolution's kernel, the real DFT is two matmuls so the complex intermediate never exists, and the LFR stacking is `lfr_m` depthwise convolutions. Verified against `torchaudio` at 218 lengths covering every residue, worst relative error 6e-6, and 3.7e-07 at f64. Also found: **the reference front end is stochastic by default** (`dither=1.0`), so the oracle must disable it or it grades noise; and the four-row prompt carries two real knobs (language, inverse text normalization -- `withitn` is the difference between punctuated, cased output and neither), so `prompt_ids` is a graph INPUT with a DEFAULTED driver binding rather than a baked constant. The languages are published as `loom.text.languages` and a `sanm.` name->row table, deliberately NOT as `loom.asr.language_ids`, which is read into `AsrDecodeTable` as decoder prompt tokens for a mechanism this family does not have. **The real cost was four entries in the exporter's own shape walk**, two of them for ops the exporter's OWN lowering passes emit (`loom_scale`, `loom_broadcast_to`), plus `reduce_sum`'s `keep_dims=True` branch and a `fill` shaped by `shape(x)`; each one silently substituted the root axis and declared one row per audio SAMPLE (loom.cpp Retro-048). `fill_like` had no ggml mapping at all and now does. Verified against FunASR on the LOGITS at three lengths: 187 frames of real speech (max \|Δ\| 3.0e-04, cosine 1.000000000, 187/187 argmax) and both 96-frame halves (8.6e-05, 2.3e-05, 96/96 each), sabotage arm 30.7 with 68/96 argmaxes still agreeing. The f64 arm inverts the headline: FunASR's own f32 is 3.4e-04 from its f64 self where loom is 5.7e-05, so the export is six times closer to the truth than the reference. **SECOND LEAF DONE (P5, 2026-09-16): `paraformer_export.py`**, two phases, claimed by name so the two leaves cannot take each other's checkpoints. It is the first model here whose output LENGTH depends on the VALUES: a CIF predictor fires a token each time a running sum of `alpha`s crosses an integer. The host decides the boundary and hands the graph the whole `(n_tokens, n_frames)` LINEAR RESAMPLING MATRIX -- every token is a weighted sum of encoder frames -- so the graph is one matmul with no cumsum, no gather and no threshold. Forced as well as preferred: `ggml_cumsum` only sums over `ne[0]` and `ggml_get_rows` needs a contiguous source. **It needed no new engine binding** (`OutputRef`, `loom.get_output`, `loom.output_shape` already existed) and reuses family 12's `TokenLabelsEpilogue`, adding one driver component (`cif_boundary`) and two `loom_lua` functions. The cost was numerical, not structural: FunASR accumulates at f64, CASTS TO F32, then floors, and computes the remainder as `(indicator + prefix_sum) - floor(prefix_sum)` in f32 LEFT TO RIGHT -- so a pure-f64 host is more accurate and puts tokens on the wrong frames, and `1 + 31.999998` rounds to exactly 33, collapsing a remainder from ~1 to 0 (loom.cpp Retro-049). Verified on the encoder TENSOR (4.48e-05 / 4.81e-06, cosine ~1, sabotage 3.81e-01) and on the transcript, character-for-character identical to FunASR on both a Chinese and an English clip. **The vocabulary landed too (2026-09-16)**: `loom::FunasrVocab` under `tokenizer.ggml.model == "funasr"`, because the TABLE is family 4's shape and the COMPOSITION is what is new -- `@@` is a SUFFIX meaning "I continue" where SentencePiece's U+2581 and WordPiece's `##` are PREFIXES meaning "a word starts here", duals with the same piece string in both roles, so no rewrite converts one to the other (loom.cpp ADR-036). The per-piece script is computed HERE rather than in C++: the reference's tests are per-character against Python's Unicode `isalpha()` and the vocabulary holds one character (U+2B5AF) that is alphabetic and outside the CJK block a range check would use. Verified differentially against `sentence_postprocess` over **20,000 random id sequences, 0 mismatches**, which is what caught that the `@@` test is ORTHOGONAL to the script (`f@@` is neither CJK nor Latin and is still a continuation; `9@@` is CJK, since digits and `@` count, and is not one). **Family 5 is complete** |
...
| 9 | **Flow-matching / diffusion acoustic stage** | matcha, supertonic, f5-tts, cosyvoice3 (DiT-CFM), voxcpm2 (LocDiT), kugelaudio (DiT), chatterbox (S3Gen), tada, voxtral-tts, pocket-tts (LSD), dots-tts, irodori-tts | 12 | ~12 | the ODE loop (done) + per-model preconditioning | **7 done** (`FlowMatchingSpec`). **Scope cut by the user 2026-09-26:** kugelaudio, tada, dots-tts and irodori-tts will not be added; Voxtral-4B-TTS is the eighth and last leaf, exported on the workstation. **Seventh leaf CosyVoice3 (P5, 2026-09-24): `cosyvoice3_export.py`**, Fun-CosyVoice3-0.5B-2512 -- Chatterbox's composition with every stage a sibling: a Qwen2-0.5B LM sampling FSQ tokens, a 22-layer DiT flow (F5-TTS's, with a causal position conv and a speaker column) on Chatterbox's `FlowMatchingSpec` unchanged, and CausalHiFT. Four phases (`lm`, `flow_encoder`, `estimator`, `vocoder`), 3.4 GB at F32, 6.9 GB export peak. The LM embeds its own text or speech ids by a per-row mask, so prefill and steps are one graph. Costs: `ras_sampling` needed three `loom.sample_row` options -- a nucleus measured over the WHOLE softmax, banned ids, and a caller's uniform draw, which makes the sampled decode gateable (loom.cpp ADR-047; greedy stops after five tokens); the DiT's rope rotates HEAD 0 ONLY (the copied early-F5 `AttnProcessor` applies it before the head split, so x_transformers' partial-rotary branch takes channels [0, 64)), reproduced by `_apply_rope_first_head`; the default voice is COMPUTED at export by running the reference frontend's two ONNX-only models (S3 tokenizer v3, CAMPPlus) on the checkout's prompt clip and shipped as driver weights; the causal sine source is reproduced at the frame rate it actually runs at; the tokenizer is Qwen2's BPE plus the ~290 tokens `CosyVoice3Tokenizer` adds at load, staged through transformers. The F0 predictor runs at f32 where the reference runs f64. Gated with pinned draws: 76/76 LM tokens identical, mel at the reference's own f32/f64 spread, free-running rmse 2.7e-03. **Sixth leaf VoxCPM2 (P5, 2026-09-24): `voxcpm2_export.py`**, a 2.3B diffusion-autoregressive TTS: two cached LMs (MiniCPM-4 base, 28 layers, and an 8-layer residual LM with no RoPE), a local encoder that re-embeds each 4 x 64-d latent patch, and a 12-layer local DiT integrated per patch over 10 guided Euler steps, then AudioVAE V2 at 48 kHz. Five phases (`feat_encode`, `base_lm`, `residual_lm`, `dit_step`, `vae_decode`), 9.3 GB at F32. The DiT's guidance is CFG-Zero* (a projection by the two velocities' dot product), which `run_ode` cannot express, so one Euler step is one graph and the loop is Lua over a 256-float state (loom.cpp ADR-046); the reference's f32 schedule ships as a driver weight. Costs: a `ROUND` primitive (the FSQ bottleneck), a tokenizer family (`voxcpm2_tokenizer_export.py`: rank-merged character BPE with byte fallback, the reference's Chinese character split, and `tokenizer_config.json`'s added tokens; 4992/4992 ids), the AudioVAE's OLD-style weight norm, which a fold must remove on a module that never ran forward (loom.cpp Retro-056), and the first export with TWO cached phases, whose attention slots collided in the one shared cache until `offset_cached_attention_layers` moved the second (loom.cpp Retro-057; its speech still transcribed exactly). Gated teacher-forced on latents at rmse 1.2e-06, free-running at 7.1e-06. **Fifth leaf Pocket-TTS (P5, 2026-09-24): `pocket_tts_export.py`**, the first AR loop over CONTINUOUS latents, and it needed no loop primitive: family 10's KV-cached decode driven from Lua, with the sampler replaced by one call of a one-step LSD flow head (`z * sqrt(temp)` and `x + v / n` in the graph, since the driver's LuaJIT has doubles only) and the EOS logit read back. Five phases (`text_embed`, `lm`, `step_embed`, `flow_head`, `mimi_decoder`), 405 MB at F32. Its cost was elsewhere: the voice ships as the flow LM's KV CACHE, so the engine gained `loom.seed_kv` (loom.cpp ADR-043); the text front end chunks long text into sentences and returns the chunks separated by `</s>` (`pocket_tts_tokenizer_export.py`, loom.cpp ADR-044, 7000/7000 against the reference, plus SentencePiece byte fallback that `spm_tokenizer_export` had been dropping); interleaved RoPE re-spelled in four dimensions; and Mimi's windowed mask built from a `positions` input as two outer products (ggml's SUB broadcasts one operand). Mimi decodes in ONE call (streamed vs one-shot 3.4e-06 in the reference). Gated teacher-forced at rmse 1.8e-06 (loom.cpp Retro-055: a latent loop amplifies rounding, so free-running is a drift bound). The other 25 voices are VOICE FILES (`pocket_tts_voices.py`, loom.cpp ADR-045): GGUFs of driver inputs stamped with a fingerprint of the `flow_lm` weights, which the model declares too, loaded by the engine for `loom_cli --voice` and loom-py's `voice=`; each carries its recording's licence. Each text chunk opens with a header id carrying the reference's EOS-tail guess, which needs the chunk's TEXT (ADR-044, amended). **Fourth leaf Chatterbox (P5, 2026-09-24): `chatterbox_export.py`**, the first AR-LM + flow composition in ONE GGUF, and it needed no new template: T3 (Llama-520M) is family 10's guided KV-cached decode with a private `t3_lm_uncond` stream, S3Gen is F5's `FlowMatchingSpec` (guidance, cosine caller schedule, caller noise) over a Matcha-style causal U-Net, and HiFT's NSF source takes its phase and noise draws as inputs. Six phases, one driver, the built-in voice as `driver_weights`, no Perth watermark by decision. Its cost was the sampler (`min_p`, and a per-id repetition penalty, loom.cpp Retro-053) and a text front end shipped as DATA (`chatterbox_tokenizer_export.py`, loom.cpp ADR-041: 3000/3000 ids against the reference). Gate waveform 2.5e-05 from the reference, which is its own f32/f64 floor. **Third leaf F5-TTS (P5, 2026-09-18): `f5_tts_export.py`**, four phases, and the first P5 family in seven to need an ENGINE primitive -- `loom.run_ode` learned classifier-free guidance, because F5-TTS's velocity field is `v_cond + scale*(v_cond - v_uncond)` rather than the estimator's output, and running the pair in Lua would put a whole mel spectrogram across the boundary four times per step (loom.cpp ADR-040; guidance at scale 0 is bit-identical to the unguided call, which is what protects the two leaves that already shipped). On this side it is TWO DECLARATIONS on the existing template rather than a bespoke sampler -- `FlowMatchingSpec.guidance` and `.schedule='caller'` (F5-TTS integrates a sway-reparameterised linspace, not `k/n_steps`) -- so the loop is unchanged, which is the test of whether a template still fits. **The model has no duration predictor and no phonemiser: it IN-FILLS**, so the reference clip's mel, the noise and the concatenated text all share ONE dynamic axis (the total frame count) and the driver slices the prompt's frames off before the vocoder. Three costs that were in no estimate: the conditioning length is the mel's own frame count (`n_samples//hop + 1`), not the `ref_audio_len` the output slice uses -- one frame, which 32 Euler steps amplify to max |D| 1.77 on the mel; `power=1` spectrograms lower to `reduce_l2_norm`, which had no ggml mapping (Whisper writes `abs()**2` and the square cancels the root); and x_transformers' `apply_rotary_pos_emb` contributed BOTH a negative-begin dynamic slice that exported as twice the rows at a negative offset (loom.cpp Retro-051) and a `rotate_half` that is FIVE-dimensional on a 4-D query -- replaced together by one substitution carrying precomputed cos/sin and the pair swap as a constant `(64, 64)` matmul, which also takes the estimator from 45 `COS` nodes to 1. Vocos's `ISTFTHead` builds a complex tensor for `torch.istft`, which has no torch-frontend handler at all, so it is this project's own traceable `ISTFT` (4.8e-07 against the real decode). Verified by DECOMPOSITION against the reference's own sampler: mel and both text-embedding branches BIT-IDENTICAL, the estimator 7.7e-06 (batch-1 against the reference's packed batch-2), and the integrated mel 1.42e-05 / cosine 0.99999994 over the generated frames. The text front end ships the TABLE and not the segmenter (`tokenizer.ggml.model == "f5"`): measured at 2000/2000 identical to `convert_char_to_pinyin` for ordinary prose, diverging by one inserted space for multi-character punctuation runs and hyphen-joined digit groups, and refusing CJK by name |
...
| 10 | **AR LM + neural codec TTS** | orpheus+SNAC, outetts+WavTokenizer, qwen3-tts ×4, moss-tts ×2, miotts, omnivoice, csm, dia, bark, zonos+dac, indextts, parler, vibevoice-tts ×2, lfm2-audio-tts | 16 | ~20 | **the codec decoder (family 11)** + a delay/RQ token-emission driver | **Dia and Qwen3-TTS shipped**, each with its codec. **MOSS-TTS-Local-v1.5 shipped 2026-09-25** with its codec (loom.cpp ADR-049–052): exported on the workstation (34 GB peak), codes exact against the reference |
| 11 | **Neural audio codecs / vocoders** (decode side) | DAC, dacvae, SNAC, **moss-audio-tokenizer-v2**, WavTokenizer, MioCodec, mimo-tokenizer, omnivoice-tokenizer, cosyvoice3-s3tok, qwen3-tts-tokenizer, tada-codec, tada-encoder | 11 | ~11 | — (it *is* the connector for 10) | HiFi-GAN/iSTFT done inside 7/8/9; RVQ/FSQ not started |
| 12 | **BERT-family token classifiers** | fireredpunc, fullstop-punc, punctuate-all, pcs (4 heads), bert-base | 4 | 5 | — (one linear head) | **done** (P5, 2026-09-03): `token_classification_export.py`, ONE generic recognizer claiming any `*ForTokenClassification` directory. It cost no engine primitive and one driver component (`token_labels_epilogue` = `ctc_greedy_epilogue` minus the collapse); the connector really was nothing. Verified on TWO structurally different encoders against transformers on the TENSOR — `dslim/bert-base-NER` (max \|Δ\| 1.24e-05) and `dslim/distilbert-NER` (5.72e-06), 138 tokens each; DistilBERT has no token-type embeddings, no `position_ids` argument and `.transformer` where BERT has `.encoder`, and the template absorbs all three by reading `base.forward`'s signature rather than its name. The one thing that was not free is a tracing decision — a single unpadded sequence exports with no attention mask at all, because every route transformers takes to build one bakes the traced length (loom.cpp ADR-019). **Proved on a THIRD checkpoint 2026-09-04**, `oliverguhr/fullstop-punctuation-multilang-large` — XLM-R, and the first where the TOKENIZER rather than the encoder was the new thing. Two findings, neither in the template: a fairseq-derived checkpoint's ids are NOT its SentencePiece protobuf's piece order (`transformers`' converter re-heads the vocabulary and shifts every piece by one), so the ids are now read off the `tokenizer.json` beside it and the piece types and normalizer off the protobuf (loom.cpp ADR-027); and XLM-R numbers positions from `padding_idx + 1`, so the family's 0-based `position_ids` read two rows of the table trained as padding — max |Δ| 7.040 with 24 of 27 argmaxes still agreeing (loom.cpp Retro-039). Verified over 70 tokens: max |Δ| 4.196e-05, cosine 0.999999973, 70/70 argmax, sabotage arm 17.49, engine encode identical to `AutoTokenizer` on 8/8 multilingual sentences |
...
6. **CNN+CTC** (family 4) and **SANM** (family 5) — both are family-1-shaped once the encoder template
   is generalized past NeMo. Family 4 needs no new head at all. **DONE for family 4 (P5, 2026-09-12)
   and for family 5's first leaf (P5, 2026-09-15), and half of this sentence was wrong both times.** "Needs no new head"
   was exact: the CTC epilogue, the builder and the engine's reduction are all family 1's, reused
   unchanged. "Family-1-shaped once the encoder template generalizes" was not — the encoder template
   never generalized, because family 1's trace is NeMo-shaped around a mel front end and family 4 takes
   a raw waveform with no length argument. `ctc_asr_export.py` is a SIBLING of `nemo_asr_export.py`, not
   a leaf of it, and the honest statement of what the two share is one component. What family 4 actually
   cost is in the table above.
